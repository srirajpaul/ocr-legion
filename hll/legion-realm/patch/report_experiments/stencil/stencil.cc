...
/*
 * 08_multiple_partitions.cc
 * We compute a simple 5-point
                     ...
                     Context ctx, Runtime *runtime)
{
  ...
  printf("Initializing field %d for block %d...node %d\n", fid, point,
                  ...
                  Context ctx, Runtime *runtime)
{
  ...
  {
    printf("Running slow stencil path for point %d...node %d\n", point,
      runtime->get_executing_processor(ctx).address_space());
  ...
  {
    printf("Running fast stencil path for point %d...node %d\n", point,
      runtime->get_executing_processor(ctx).address_space());
