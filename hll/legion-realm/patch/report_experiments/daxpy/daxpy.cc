                    ...
                    Context ctx, Runtime *runtime)
{
  ...
  // how many subregions we should make.
  {
    ...
    for (int i = 1+ADD_OFFSET; i < command_args.argc; i++)
    {
                     ...
                     Context ctx, Runtime *runtime)
{
  ...
  printf("Initializing field %d for block %d...on node %d\n", fid, point,
    runtime->get_executing_processor(ctx).address_space());
                ...
                Context ctx, Runtime *runtime)
{
  ...
  printf("Running daxpy computation with alpha %.8g for point %d...node %d\n",
          alpha, point, runtime->get_executing_processor(ctx).address_space());
